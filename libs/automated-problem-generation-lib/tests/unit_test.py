...
class TryParser(TestCase):
    def test_run_parser(self):
        with open(os.path.join("kosc-automated-problem-generation-lib","tests","variables.yml")) as file:
    ...
    def test_run_parser_err(self):
        with open(os.path.join("kosc-automated-problem-generation-lib","tests","variables_err.yml")) as file:
            parser_var_file(file)
        self.assertTrue(True)
